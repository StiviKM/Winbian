#!/bin/bash
# =============================================================================
# Winbian_Two.sh - Debian 13 Windows Lookalike Setup - Stage 2
...
AUTOSTART_FILE="$ACTUAL_HOME/.config/autostart/winbian_two.desktop"
rm -f "$AUTOSTART_FILE"
...
DTP_CONF="$WINBIAN_DIR/Dash_To_Panel_Win"
ARC_CONF="$WINBIAN_DIR/Arc_Menu_Win"
...
WALLPAPER_SRC="$WINBIAN_DIR/Wallpaper.jpg"
...
# Open RDP port in firewall using ufw (Debian default)
if command -v ufw &>/dev/null; then
  sudo ufw allow 3389/tcp 2>/dev/null && \
    log "Firewall: RDP port 3389 opened via ufw" || \
    log "WARNING: Could not configure ufw for RDP"
else
  log "WARNING: ufw not found, skipping firewall rule"
fi
...
color_echo "green" ""
color_echo "green" "╔══════════════════════════════════════════╗"
color_echo "green" "║   ✅ Winbian Complete! Rebooting...      ║"
color_echo "green" "║   Log saved to: ~/winbian.log            ║"
color_echo "green" "╚══════════════════════════════════════════╝"
color_echo "green" ""

kill $SUDO_KEEPALIVE_PID 2>/dev/null || true
log "Winbian Stage 2 complete. Rebooting in 15 seconds..."
echo
